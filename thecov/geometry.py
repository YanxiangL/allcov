...
class Geometry(ABC):
    def save(self, filename: str):
        ...
        self.logger.info(f"Saved to {filename} in {time.time() - start:.3f} s.")  # type: ignore
    ...
    def load(self, filename: str):
        ...
        self.__dict__.update(obj.__dict__)  # type: ignore
        self.logger.info(f"Loaded from {filename} in {time.time() - start:.3f} s.")  # type: ignore
...
class BoxGeometry(Geometry):
    ...
    def __init__(
        self, volume: float | None = None, nbar: float | np.ndarray | None = None
    ):
        self._volume = volume
        self._nbar = nbar
        self._zmin = None
    ...
    @property
    def nbar(self) -> float | np.ndarray:
        assert self._nbar is not None, "nbar must be set to compute shotnoise."
        return self._nbar

    @nbar.setter
    def nbar(self, nbar: float | np.ndarray):
        self._nbar = nbar

    @property
    def shotnoise(self) -> float | np.ndarray:
        ...
        return 1.0 / self.nbar

    @shotnoise.setter
    def shotnoise(self, shotnoise: float | np.ndarray):
        ...
        self.nbar = 1.0 / shotnoise

    @property
    def zedges(self) -> np.ndarray:
        return self._zedges

    @property
    def zmid(self) -> np.ndarray:
    ...
    def set_effective_volume(
        self, zmin: float, zmax: float, fsky: float | None = None
    ) -> Optional[float]:
    ...
    def set_nz(self, zedges: np.ndarray, nz: np.ndarray, *args, **kwargs):
        ...
        *args, **kwargs
        ...
        assert len(zedges) == len(nz) + 1, (
            "Length of zedges should equal length of nz + 1."
        )
        ...
        bin_volume = self.fsky * np.diff(
            self.cosmo.comoving_radial_distance(self.zedges) ** 3
        )
    ...
    def set_randoms(
        ...
        bins : array_like, optional
            Array of redshift bin edges to be used for the estimation of the redshift distribution.
        fsky : float, optional
            Fraction of the sky covered by the sample. If not given, the current value of fsky is used.
        ...
        # Interpolate nz for different redshift bins, and set the effective volume and number density of the box accordingly.
        nz_hist = RedshiftDensityInterpolator(
            z=randoms["Z"],
            bins=bins,
            fsky=self.fsky,
    ...
    @area.setter
    def area(self, area: float | np.ndarray):
        self.fsky = area / 360**2 * np.pi

    @property
    def nz(self) -> float | np.ndarray:
        return self._nz

    @property
    def ngals(self) -> float | np.ndarray:
        assert self.nbar is not None and self.volume is not None, (
            "nbar and volume must be set to compute ngals."
        )
...
class SurveyGeometry(Geometry, base.FourierBinned):
    ...
    cellsize : float
        Size of the cells in the mesh. If not given, it is set to boxsize/nmesh.
    boxsize : float
        Size of the box.
    boxpad : float
        Box padding.
    ...
    def __init__(
        ...
        boxpad: float = 2.0,
        kmax_window: float = 0.02,
        kmodes_sampled: int = 5000,
        alpha: float = 1.0,
        nthreads: int | None = None,
        resume_file: str | None = None,
        tqdm: Callable = shell_tqdm,
        **kwargs,
    ):
        ...
        if self.nthreads is None:
            self.nthreads = int(os.environ.get("OMP_NUM_THREADS", os.cpu_count() or 1))
        ...
        for name in ["WEIGHT", "WEIGHT_FKP"]:
            # If no weight column is provided, set it to 1.0 for all objects.
            if name not in self._randoms:
                self._randoms[name] = np.ones(self._randoms.size, dtype="f8")
        if "NZ" not in self._randoms:
            ...
            nside = 512
            distance = np.sqrt(np.sum(np.array(randoms["POSITION"]) ** 2, axis=-1))
            xyz = np.array(randoms["POSITION"]) / distance[:, None]
            ...
            self._randoms["NZ"] = self.alpha * np.array(nbar(distance))
        if nmesh is None and cellsize is None:
            # Pick value that will give at least k_mask = kmax_window in the FFTs
            cellsize = np.pi / kmax_window / (1.0 + 1e-9)

        self._mesh = CatalogMesh(
            data_positions=self._randoms["POSITION"],
            data_weights=self._randoms["WEIGHT"],
            position_type="pos",
            nmesh=nmesh,
            cellsize=cellsize,
            boxsize=boxsize,
        ...
        self.logger.info(
            f"Using box size {self._mesh.boxsize}, box center {self._mesh.boxcenter} and nmesh {self._mesh.nmesh}."
        )
        self.boxsize = self._mesh.boxsize[0]
        self.nmesh = self._mesh.nmesh[0]
        assert np.allclose(self._mesh.boxsize, self.boxsize) and np.all(
            self._mesh.nmesh == self.nmesh
        )
        ...
        if self.knyquist < kmax_window:
            self.logger.warning(
                f"Nyquist wavelength {self.knyquist} smaller than required kmax_window = {kmax_window}."
            )

        data_size = getattr(self._mesh, "data_size", None)

        self.logger.info(f"Average of {data_size / self.nmesh**3} objects per voxel.")
    ...
    @property
    def knyquist(self) -> float:
        return np.pi * self.nmesh / self.boxsize

    # Duplicate of kfun around line 608
    # @property
    # def kfun(self) -> float:  # type: ignore
    ...
    def W_cat(self, W):
        ...
        if f"W{w}" not in self._randoms.columns():
            self._randoms[f"W{w}"] = (
                np.array(self._randoms["NZ"]) ** (int(w[0]) - 1)
                * np.array(self._randoms["WEIGHT_FKP"]) ** int(w[1])
                * np.array(self._randoms["WEIGHT"])
                * self.alpha
            )
    ...
    def I(self, W):
        ...
        self.W_cat(w)
        return np.array(self._randoms[f"W{w}"]).sum().tolist()
    ...
    @property
    def ikgrid(self) -> list[np.ndarray]:
    ...
    def get_mesh(self, label: str):
        ...
        return self._mesh.clone(
            data_positions=self.randoms["POSITION"],
            data_weights=weights,
            position_type="pos",
        ).to_mesh(compensate=True)

    def get_fft(self, label: str):
        ...
        -------
            array_like
                FFT of the window function Wij."""
        toret = self.get_mesh(label).r2c()  # type: ignore
        toret *= self.nmesh**3
        return toret.value

    def compute_pypower(self, label1: str, label2: str | None = None, *args, **kwargs):
        """Computes the power spectrum of the window function Wij using pypower.

        Parameters
        ----------
        label1 : str
            First window function label.
        label2 : str, optional
            Second window function label. If None, label1 is used.
        *args
            Additional positional arguments passed to MeshFFTPower.
        **kwargs
            Additional keyword arguments passed to MeshFFTPower.
        Returns
        -------
            MeshFFTPower
                Power spectrum of the window function Wij.
        """
        if label2 is None:
            label2 = label1
    ...
    def compute_power(self, label1: str, label2: str | None = None, kedges="auto"):
        """Computes the power spectrum of the window function Wij.
        ...
        label2 : str, optional
            Second window function label. If None, label1 is used.
        kedges : array_like or str, optional
            Edges of the k-bins. If "auto", they are automatically determined.

        Returns
        -------
        tuple
            Tuple containing the k-bin edges and the power spectrum.
        """
        if label2 is None:
            label2 = label1
        ...
        ibin = np.digitize(kpower, np.array(kedges))

        pk = np.array([power[ibin == i].mean() for i in range(1, len(kedges))])
        pk = pk.real if label1 == label2 else pk  # type: ignore
    ...
    def compute_Qmus(self, W1, W2=None, sedges=None, *args, **kwargs):
        ...
        # I think this should be the correct one. The line above is outdated with the latest pycorr version.
        from pycorr.twopoint_counter import BaseTwoPointCounter

        muedges = np.linspace(-1, 1.0, 201)

        edges = (np.geomspace(1, 3400.0, 441) if sedges is None else sedges, muedges)

        pos = np.array(self.randoms["POSITION"]).T
        ...
        v = 2.0 / 3.0 * np.pi * edges[0][:, None] ** 3 * edges[1]
    ...
    def compute_cartesian_ffts(self, W: str):
        ...
        x = np.array(self.randoms["POSITION"]).T

        with self.tqdm(total=22, desc=f"Computing moments of W{w}") as pbar:
            self.set_cartesian_fft(f"W{w}", self.get_fft(f"W{w}"))
            pbar.update(1)

            for (i, i_label), (j, j_label) in itt.combinations_with_replacement(
                enumerate(["x", "y", "z"]), r=2
            ):
                # Calculates the FFT of the window function Wij multiplied by x_i x_j, where i and j can be x, y or z.
                # The result is stored in self._W with the label W{w}{i_label}{j_label}.
            ...
            for (i, i_label), (j, j_label), (k, k_label), (
                l,
                l_label,
            ) in itt.combinations_with_replacement(enumerate(["x", "y", "z"]), r=4):
    ...
    def set_cartesian_fft(self, label: str, W: np.ndarray):
        ...
        self._W[w] = (
            np.frombuffer(mp.RawArray("d", 2 * int(self.nmesh) ** 3))
            .view(np.complex128)
            .reshape(self.nmesh, self.nmesh, self.nmesh)
        )
        self._W[w][...] = W

    @property
    def randoms(self) -> Catalog:
    ...
    def compute_window_power(self):
        """Computes the window function Wij needed for the Gaussian covariance calculation, and their power spectra."""
        ...
        W22_L2 = (
            ...
            * (
                ...
                + self.W("22yy") * ky**2
                + self.W("22zz") * kz**2
                + 2.0 * self.W("22xy") * kx * ky
                + 2.0 * self.W("22yz") * ky * kz
                + 2.0 * self.W("22xz") * kz * kx
            )
            - 0.5 * W22_L0
        )
        ...
        W22_L4 = (
            ...
            * (
                ...
                + self.W("22yyyy") * ky**4
                + self.W("22zzzz") * kz**4
                + 4.0 * self.W("22xxxy") * kx**3 * ky
                + 4.0 * self.W("22xxxz") * kx**3 * kz
                + 4.0 * self.W("22xyyy") * ky**3 * kx
                + 4.0 * self.W("22yyyz") * ky**3 * kz
                + 4.0 * self.W("22xzzz") * kz**3 * kx
                + 4.0 * self.W("22yzzz") * kz**3 * ky
                + 6.0 * self.W("22xxyy") * kx**2 * ky**2
                + 6.0 * self.W("22xxzz") * kx**2 * kz**2
                + 6.0 * self.W("22yyzz") * ky**2 * kz**2
                + 12.0 * self.W("22xxyz") * kx**2 * ky * kz
                + 12.0 * self.W("22xyyz") * ky**2 * kx * kz
                + 12.0 * self.W("22xyzz") * kz**2 * kx * ky
            )
            - 5.0 / 2.0 * W22_L2
            - 7.0 / 8.0 * W22_L0
        )
        ...
        W10_L2 = (
            ...
            * (
                ...
                + self.W("10yy") * ky**2
                + self.W("10zz") * kz**2
                + 2.0 * self.W("10xy") * kx * ky
                + 2.0 * self.W("10yz") * ky * kz
                + 2.0 * self.W("10xz") * kz * kx
            )
            - 0.5 * W10_L0
        )
        ...
        W10_L4 = (
            ...
            * (
                ...
                + self.W("10yyyy") * ky**4
                + self.W("10zzzz") * kz**4
                + 4.0 * self.W("10xxxy") * kx**3 * ky
                + 4.0 * self.W("10xxxz") * kx**3 * kz
                + 4.0 * self.W("10xyyy") * ky**3 * kx
                + 4.0 * self.W("10yyyz") * ky**3 * kz
                + 4.0 * self.W("10xzzz") * kz**3 * kx
                + 4.0 * self.W("10yzzz") * kz**3 * ky
                + 6.0 * self.W("10xxyy") * kx**2 * ky**2
                + 6.0 * self.W("10xxzz") * kx**2 * kz**2
                + 6.0 * self.W("10yyzz") * ky**2 * kz**2
                + 12.0 * self.W("10xxyz") * kx**2 * ky * kz
                + 12.0 * self.W("10xyyz") * ky**2 * kx * kz
                + 12.0 * self.W("10xyzz") * kz**2 * kx * ky
            )
            - 5.0 / 2.0 * W10_L2
            - 7.0 / 8.0 * W10_L0
        )
        ...
        power = [
            A * np.conj(B)
            for A, B in self.tqdm(
                utils.limit(itt.combinations_with_replacement(poles, 2), 16),
                total=16,
        ...
        power = self.tqdm(power, total=17, desc="Averaging power spectra over bins.")

        self._window_power = np.array(
            [
                [p[ibin == i].real.mean() for i in range(1, self.kbins + 1)]
                for p in power
            ]
        )
        ...
        for i, (w1, w2) in enumerate(
            itt.combinations_with_replacement(["22", "22", "22", "10", "10", "10"], 2)
        ):
            if i > 15:
                break
            self._window_power[i] /= self.I(w1) * self.I(w2)  # normalizing

        for i, (l1, l2) in utils.limit(
            enumerate(itt.combinations_with_replacement(2 * [0, 2, 4], 2)), 16
        ):
            if i > 15:
                break
        ...
        if self._resume_file is not None:
            raise NotImplementedError("Saving resume file is not implemented yet. ")
    ...
    def get_window_power_interpolators(self):
        ...
        assert self._window_power is not None, (
            "Window power spectra have not been computed yet."
        )
        kavg = self._window_power[-1]
        return [
            InterpolatedUnivariateSpline(kavg, Pwin) for Pwin in self._window_power[:-1]
        ]
    ...
    def compute_window_kernels(self):
        ...
        if self.WinKernel is None and self.WinKernel_error is None:
            ...
            self.WinKernel_error = np.empty(
                [self.kbins, 2 * self.delta_k_max + 1, 15, 6]
            )
            self.WinKernel_error.fill(np.nan)

        ell_factor = (
            lambda l1, l2: (2 * l1 + 1) * (2 * l2 + 1) * (2 if 0 in (l1, l2) else 1)
        )
        assert self.WinKernel is not None and self.WinKernel_error is not None, (
            "Window kernels have not been initialized."
        )
        assert self.kmin is not None and self.dk is not None, (
            "k bins have not been set."
        )
        ...
        for i, km in self.tqdm(
            enumerate(kmodes), desc="Computing window kernels", total=self.kbins
        ):
            if self._resume_file is not None:
            ...
            with mp.Pool(
                processes=min(self.nthreads, len(chunks)),
                initializer=init_worker,
                initargs=[*[self._W[w] for w in W_LABELS], init_params],
            ) as pool:
                ...
                for k2_bin_index in range(0, 2 * self.delta_k_max + 1):
                    if (
                        k2_bin_index + i - self.delta_k_max >= self.kbins
                        or k2_bin_index + i - self.delta_k_max < 0
                    ):
                        self.WinKernel[i, k2_bin_index, :, :] = 0
                    else:
                        self.WinKernel[i, k2_bin_index, :, :] /= Nmodes[
                            i + k2_bin_index - self.delta_k_max
                        ]
    ...
    def _compute_window_kernel_row(bin_kmodes):
        ...
        for ik1x, ik1y, ik1z, ik1r in bin_kmodes:
            ...
            xx = (
                ...
                + W["22yy"] * k1yh**2
                + W["22zz"] * k1zh**2
                + 2.0 * W["22xy"] * k1xh * k1yh
                + 2.0 * W["22yz"] * k1yh * k1zh
                + 2.0 * W["22xz"] * k1zh * k1xh
            )
            ...
            W_k2L2 = (
                ...
                * (
                    ...
                    + W["22yy"] * k2yh**2
                    + W["22zz"] * k2zh**2
                    + 2.0 * W["22xy"] * k2xh * k2yh
                    + 2.0 * W["22yz"] * k2yh * k2zh
                    + 2.0 * W["22xz"] * k2zh * k2xh
                )
                - 0.5 * W["22"]
            )
            ...
            W_k1L4 = (
                ...
                * (
                    ...
                    + W["22yyyy"] * k1yh**4
                    + W["22zzzz"] * k1zh**4
                    + 4.0 * W["22xxxy"] * k1xh**3 * k1yh
                    + 4.0 * W["22xxxz"] * k1xh**3 * k1zh
                    + 4.0 * W["22xyyy"] * k1yh**3 * k1xh
                    + 4.0 * W["22yyyz"] * k1yh**3 * k1zh
                    + 4.0 * W["22xzzz"] * k1zh**3 * k1xh
                    + 4.0 * W["22yzzz"] * k1zh**3 * k1yh
                    + 6.0 * W["22xxyy"] * k1xh**2 * k1yh**2
                    + 6.0 * W["22xxzz"] * k1xh**2 * k1zh**2
                    + 6.0 * W["22yyzz"] * k1yh**2 * k1zh**2
                    + 12.0 * W["22xxyz"] * k1xh**2 * k1yh * k1zh
                    + 12.0 * W["22xyyz"] * k1yh**2 * k1xh * k1zh
                    + 12.0 * W["22xyzz"] * k1zh**2 * k1xh * k1yh
                )
                - 5.0 / 2.0 * W_k1L2
                - 7.0 / 8.0 * W_L0
            )
            ...
            k1k2 = (
                W["22xxxx"] * (k1xh * k2xh) ** 2
                + W["22yyyy"] * (k1yh * k2yh) ** 2
                + W["22zzzz"] * (k1zh * k2zh) ** 2
                ...
                * (
                    k1xh**2 * k2yh**2
                    + k1yh**2 * k2xh**2
                    + 4.0 * k1xh * k1yh * k2xh * k2yh
                )
                + W["22xxzz"]
                * (
                    k1xh**2 * k2zh**2
                    + k1zh**2 * k2xh**2
                    + 4.0 * k1xh * k1zh * k2xh * k2zh
                )
                + W["22yyzz"]
                * (
                    k1yh**2 * k2zh**2
                    + k1zh**2 * k2yh**2
                    + 4.0 * k1yh * k1zh * k2yh * k2zh
                )
                + W["22xyyz"]
                * (
                    k1xh * k1zh * k2yh**2
                    + k1yh**2 * k2xh * k2zh
                    + 2.0 * k1yh * k2yh * (k1zh * k2xh + k1xh * k2zh)
                )
                * 2
                + W["22xxyz"]
                * (
                    k1yh * k1zh * k2xh**2
                    + k1xh**2 * k2yh * k2zh
                    + 2.0 * k1xh * k2xh * (k1zh * k2yh + k1yh * k2zh)
                )
                * 2
                + W["22xyzz"]
                * (
                    k1yh * k1xh * k2zh**2
                    + k1zh**2 * k2yh * k2xh
                    + 2.0 * k1zh * k2zh * (k1xh * k2yh + k1yh * k2xh)
                )
                * 2
            )
            ...
            W_k2L4 = (
                ...
                * (
                    ...
                    + W["22yyyy"] * k2yh**4
                    + W["22zzzz"] * k2zh**4
                    + 4.0 * W["22xxxy"] * k2xh**3 * k2yh
                    + 4.0 * W["22xxxz"] * k2xh**3 * k2zh
                    + 4.0 * W["22xyyy"] * k2yh**3 * k2xh
                    + 4.0 * W["22yyyz"] * k2yh**3 * k2zh
                    + 4.0 * W["22xzzz"] * k2zh**3 * k2xh
                    + 4.0 * W["22yzzz"] * k2zh**3 * k2yh
                    + 6.0 * W["22xxyy"] * k2xh**2 * k2yh**2
                    + 6.0 * W["22xxzz"] * k2xh**2 * k2zh**2
                    + 6.0 * W["22yyzz"] * k2yh**2 * k2zh**2
                    + 12.0 * W["22xxyz"] * k2xh**2 * k2yh * k2zh
                    + 12.0 * W["22xyyz"] * k2yh**2 * k2xh * k2zh
                    + 12.0 * W["22xyzz"] * k2zh**2 * k2xh * k2yh
                )
                - 5.0 / 2.0 * W_k2L2
                - 7.0 / 8.0 * W_L0
            )
            ...
            W_k1L2_k2L2 = 9.0 / 4.0 * k1k2 - 3.0 / 4.0 * xx - 1.0 / 2.0 * W_k2L2
            # approximate as 6th order FFTs not simulated
            W_k1L2_k2L4 = 2 / 7.0 * W_k1L2 + 20 / 77.0 * W_k1L4
            W_k1L4_k2L2 = W_k1L2_k2L4  # approximate
            W_k1L4_k2L4 = 1 / 9.0 * W_L0 + 100 / 693.0 * W_k1L2 + 162 / 1001.0 * W_k1L4
            ...
            k1k2W12 = (
                np.conj(W["12xxxx"]) * (k1xh * k2xh) ** 2
                + np.conj(W["12yyyy"]) * (k1yh * k2yh) ** 2
                + np.conj(W["12zzzz"]) * (k1zh * k2zh) ** 2
                + np.conj(W["12xxxy"])
                * (k1xh * k1yh * k2xh**2 + k1xh**2 * k2xh * k2yh)
                * 2
                + np.conj(W["12xxxz"])
                * (k1xh * k1zh * k2xh**2 + k1xh**2 * k2xh * k2zh)
                * 2
                + np.conj(W["12yyyz"])
                * (k1yh * k1zh * k2yh**2 + k1yh**2 * k2yh * k2zh)
                * 2
                + np.conj(W["12yzzz"])
                * (k1zh * k1yh * k2zh**2 + k1zh**2 * k2zh * k2yh)
                * 2
                + np.conj(W["12xyyy"])
                * (k1yh * k1xh * k2yh**2 + k1yh**2 * k2yh * k2xh)
                * 2
                + np.conj(W["12xzzz"])
                * (k1zh * k1xh * k2zh**2 + k1zh**2 * k2zh * k2xh)
                * 2
                + np.conj(W["12xxyy"])
                * (
                    k1xh**2 * k2yh**2
            ...
            xxW12 = (
                np.conj(W["12xx"]) * k1xh**2
                + np.conj(W["12yy"]) * k1yh**2
                + np.conj(W["12zz"]) * k1zh**2
                + 2.0 * np.conj(W["12xy"]) * k1xh * k1yh
                + 2.0 * np.conj(W["12yz"]) * k1yh * k1zh
                + 2.0 * np.conj(W["12xz"]) * k1zh * k1xh
            )
            ...
            W12_k1L4 = (
                ...
                / 8.0
                * (
                    np.conj(W["12xxxx"]) * k1xh**4
                    + np.conj(W["12yyyy"]) * k1yh**4
                    + np.conj(W["12zzzz"]) * k1zh**4
                    + 4.0 * np.conj(W["12xxxy"]) * k1xh**3 * k1yh
                    + 4.0 * np.conj(W["12xxxz"]) * k1xh**3 * k1zh
                    + 4.0 * np.conj(W["12xyyy"]) * k1yh**3 * k1xh
                    + 6.0 * np.conj(W["12xxyy"]) * k1xh**2 * k1yh**2
                    + 6.0 * np.conj(W["12xxzz"]) * k1xh**2 * k1zh**2
                    + 6.0 * np.conj(W["12yyzz"]) * k1yh**2 * k1zh**2
                    + 12.0 * np.conj(W["12xxyz"]) * k1xh**2 * k1yh * k1zh
                    + 12.0 * np.conj(W["12xyyz"]) * k1yh**2 * k1xh * k1zh
                    + 12.0 * np.conj(W["12xyzz"]) * k1zh**2 * k1xh * k1yh
                )
                - 5.0 / 2.0 * W12_k1L2
                - 7.0 / 8.0 * W12c_L0
            )

            W12_k1L4_k2L2 = 2 / 7.0 * W12_k1L2 + 20 / 77.0 * W12_k1L4
            W12_k1L4_k2L4 = (
                1 / 9.0 * W12c_L0 + 100 / 693.0 * W12_k1L2 + 162 / 1001.0 * W12_k1L4
            )

            W12_k2L2 = 1.5 * (
                np.conj(W["12xx"]) * k2xh**2
                + np.conj(W["12yy"]) * k2yh**2
                + np.conj(W["12zz"]) * k2zh**2
                + 2.0 * np.conj(W["12xy"]) * k2xh * k2yh
                + 2.0 * np.conj(W["12yz"]) * k2yh * k2zh
                + 2.0 * np.conj(W["12xz"]) * k2zh * k2xh
            ) - 0.5 * np.conj(W["12"])
            ...
            W12_k2L4 = (
                ...
                / 8.0
                * (
                    np.conj(W["12xxxx"]) * k2xh**4
                    + np.conj(W["12yyyy"]) * k2yh**4
                    + np.conj(W["12zzzz"]) * k2zh**4
                    + 4.0 * np.conj(W["12xxxy"]) * k2xh**3 * k2yh
                    + 4.0 * np.conj(W["12xxxz"]) * k2xh**3 * k2zh
                    + 4.0 * np.conj(W["12xyyy"]) * k2yh**3 * k2xh
                    + 4.0 * np.conj(W["12yyyz"]) * k2yh**3 * k2zh
                    + 4.0 * np.conj(W["12xzzz"]) * k2zh**3 * k2xh
                    + 4.0 * np.conj(W["12yzzz"]) * k2zh**3 * k2yh
                    + 6.0 * np.conj(W["12xxyy"]) * k2xh**2 * k2yh**2
                    + 6.0 * np.conj(W["12xxzz"]) * k2xh**2 * k2zh**2
                    + 6.0 * np.conj(W["12yyzz"]) * k2yh**2 * k2zh**2
                    + 12.0 * np.conj(W["12xxyz"]) * k2xh**2 * k2yh * k2zh
                    + 12.0 * np.conj(W["12xyyz"]) * k2yh**2 * k2xh * k2zh
                    + 12.0 * np.conj(W["12xyzz"]) * k2zh**2 * k2xh * k2yh
                )
                - 5.0 / 2.0 * W12_k2L2
                - 7.0 / 8.0 * W12c_L0
            )
            ...
            W12_k1L2_k2L2 = (
                9.0 / 4.0 * k1k2W12 - 3.0 / 4.0 * xxW12 - 1.0 / 2.0 * W12_k2L2
            )

            W_k1L2_Sumk2L22 = (
                1 / 5.0 * W_k1L2 + 2 / 7.0 * W_k1L2_k2L2 + 18 / 35.0 * W_k1L2_k2L4
            )
            W_k1L2_Sumk2L24 = 2 / 7.0 * W_k1L2_k2L2 + 20 / 77.0 * W_k1L2_k2L4
            W_k1L4_Sumk2L22 = (
                1 / 5.0 * W_k1L4 + 2 / 7.0 * W_k1L4_k2L2 + 18 / 35.0 * W_k1L4_k2L4
            )
            W_k1L4_Sumk2L24 = 2 / 7.0 * W_k1L4_k2L2 + 20 / 77.0 * W_k1L4_k2L4
            W_k1L4_Sumk2L44 = (
                1 / 9.0 * W_k1L4
                + 100 / 693.0 * W_k1L4_k2L2
                + 162 / 1001.0 * W_k1L4_k2L4
            )
            ...
            C00exp += [
                ...
                W_k2L2 * W12c_L0,
                W_k2L4 * W12c_L0,
                np.conj(W12c_L0) * W12c_L0,
            ]

            C22exp = [
                Wc_k2L2 * W_k1L2 + Wc_L0 * W_k1L2_k2L2,
            ...
            C22exp += [
                ...
                0.5
                * (
                    (1 / 5.0 * W_L0 + 2 / 7.0 * W_k1L2 + 18 / 35.0 * W_k1L4) * W12_k2L2
                    + W_k1L2_k2L2 * W12_k1L2
                    + (
                        1 / 5.0 * W_k2L2
                        + 2 / 7.0 * W_k1L2_k2L2
                        + 18 / 35.0 * W_k1L4_k2L2
                    ...
                    * W12c_L0
                    + W_k1L2 * W12_k1L2_k2L2
                ),
                0.5
                * (
                    (2 / 7.0 * W_k1L2 + 20 / 77.0 * W_k1L4) * W12_k2L2
                    + W_k1L4_k2L2 * W12_k1L2
                    + (2 / 7.0 * W_k1L2_k2L2 + 20 / 77.0 * W_k1L4_k2L2) * W12c_L0
                    + W_k1L4 * W12_k1L2_k2L2
                ),
                0.5
                * (
                    W_k1L2_k2L2 * W12_k2L2
                    + (1 / 5.0 * W_L0 + 2 / 7.0 * W_k2L2 + 18 / 35.0 * W_k2L4)
                    * W12_k1L2
                    + (
                        1 / 5.0 * W_k1L2
                        + 2 / 7.0 * W_k1L2_k2L2
                    ...
                    * W12c_L0
                    + W_k2L2 * W12_k1L2_k2L2
                ),
                0.5
                * (
                    W_k1L2_k2L4 * W12_k2L2
                    + (2 / 7.0 * W_k2L2 + 20 / 77.0 * W_k2L4) * W12_k1L2
                    + W_k2L4 * W12_k1L2_k2L2
                    + (2 / 7.0 * W_k1L2_k2L2 + 20 / 77.0 * W_k1L2_k2L4) * W12c_L0
                ),
                np.conj(W12_k1L2_k2L2) * W12c_L0 + np.conj(W12_k1L2) * W12_k2L2,
            ]

            C44exp = [
                Wc_k2L4 * W_k1L4 + Wc_L0 * W_k1L4_k2L4,
            ...
            C44exp += [
                ...
                0.5
                * (
                    (2 / 7.0 * W_k1L2 + 20 / 77.0 * W_k1L4) * W12_k2L4
                    + W_k1L2_k2L4 * W12_k1L4
                    + (2 / 7.0 * W_k1L2_k2L4 + 20 / 77.0 * W_k1L4_k2L4) * W12c_L0
                    + W_k1L2 * W12_k1L4_k2L4
                ),
                0.5
                * (
                    (1 / 9.0 * W_L0 + 100 / 693.0 * W_k1L2 + 162 / 1001.0 * W_k1L4)
                    * W12_k2L4
                    + W_k1L4_k2L4 * W12_k1L4
                    + (
                        1 / 9.0 * W_k2L4
                        + 100 / 693.0 * W_k1L2_k2L4
                        + 162 / 1001.0 * W_k1L4_k2L4
                    )
                    * W12c_L0
                    + W_k1L4 * W12_k1L4_k2L4
                ),
                0.5
                * (
                    W_k1L4_k2L2 * W12_k2L4
                    + (2 / 7.0 * W_k2L2 + 20 / 77.0 * W_k2L4) * W12_k1L4
                    + W_k2L2 * W12_k1L4_k2L4
                    + (2 / 7.0 * W_k1L4_k2L2 + 20 / 77.0 * W_k1L4_k2L4) * W12c_L0
                ),
                0.5
                * (
                    W_k1L4_k2L4 * W12_k2L4
                    + (1 / 9.0 * W_L0 + 100 / 693.0 * W_k2L2 + 162 / 1001.0 * W_k2L4)
                    * W12_k1L4
                    + W_k2L4 * W12_k1L4_k2L4
                    + (
                        1 / 9.0 * W_k1L4
                        + 100 / 693.0 * W_k1L4_k2L2
                        + 162 / 1001.0 * W_k1L4_k2L4
                ...
                np.conj(W12_k1L4_k2L4) * W12c_L0 + np.conj(W12_k1L4) * W12_k2L4,
            ]  # 1/(nbar)^2
            ...
            C20exp += [
                W_k1L2 * W12c_L0 + W["22"] * W12_k1L2,
                0.5
                * (
                    (1 / 5.0 * W["22"] + 2 / 7.0 * W_k1L2 + 18 / 35.0 * W_k1L4)
                    * W12c_L0
                    + W_k1L2 * W12_k1L2
                ),
                0.5
                * (
                    (2 / 7.0 * W_k1L2 + 20 / 77.0 * W_k1L4) * W12c_L0
                    + W_k1L4 * W12_k1L2
                ),
            ...
            C40exp += [
                ...
                * (
                ...
                0.5
                * (
                    (1 / 9.0 * W["22"] + 100 / 693.0 * W_k1L2 + 162 / 1001.0 * W_k1L4)
                    * W12c_L0
                    + W_k1L4 * W12_k1L4
                ),
            ...
            C42exp = [
                Wc_k2L2 * W_k1L4 + Wc_L0 * W_k1L4_k2L2,
            ...
            C42exp += [
                ...
                * (
                    (2 / 7.0 * W_k1L2 + 20 / 77.0 * W_k1L4) * W12_k2L2
                    + W_k1L2_k2L2 * W12_k1L4
                    + (2 / 7.0 * W_k1L2_k2L2 + 20 / 77.0 * W_k1L4_k2L2) * W12c_L0
                    + W_k1L2 * W12_k1L4_k2L2
                ),
                0.5
                * (
                    (1 / 9.0 * W["22"] + 100 / 693.0 * W_k1L2 + 162 / 1001.0 * W_k1L4)
                    * W12_k2L2
                    + W_k1L4_k2L2 * W12_k1L4
                    + (
                        1 / 9.0 * W_k2L2
                        + 100 / 693.0 * W_k1L2_k2L2
                        + 162 / 1001.0 * W_k1L4_k2L2
                    )
                    * W12c_L0
                    + W_k1L4 * W12_k1L4_k2L2
                ),
                0.5
                * (
                    W_k1L4_k2L2 * W12_k2L2
                    + (1 / 5.0 * W["22"] + 2 / 7.0 * W_k2L2 + 18 / 35.0 * W_k2L4)
                    * W12_k1L4
                    + W_k2L2 * W12_k1L4_k2L2
                    + (
                        1 / 5.0 * W_k1L4
                        + 2 / 7.0 * W_k1L4_k2L2
                        + 18 / 35.0 * W_k1L4_k2L4
                ...
                0.5
                * (
                    W_k1L4_k2L4 * W12_k2L2
                    + (2 / 7.0 * W_k2L2 + 20 / 77.0 * W_k2L4) * W12_k1L4
                    + W_k2L4 * W12_k1L4_k2L2
                    + (2 / 7.0 * W_k1L4_k2L2 + 20 / 77.0 * W_k1L4_k2L4) * W12c_L0
                ),
                np.conj(W12_k1L4_k2L2) * W12c_L0 + np.conj(W12_k1L4) * W12_k2L2,
            ]  # 1/(nbar)^2
            ...
            for delta_k in range(-delta_k_max, delta_k_max + 1):
                ...
                for term in range(15):
                    WinKernel[delta_k + delta_k_max, term, 0] += np.sum(
                        np.real(C00exp[term][modes])
                    )
                    WinKernel[delta_k + delta_k_max, term, 1] += np.sum(
                        np.real(C22exp[term][modes])
                    )
                    WinKernel[delta_k + delta_k_max, term, 2] += np.sum(
                        np.real(C44exp[term][modes])
                    )
                    WinKernel[delta_k + delta_k_max, term, 3] += np.sum(
                        np.real(C20exp[term][modes])
                    )
                    WinKernel[delta_k + delta_k_max, term, 4] += np.sum(
                        np.real(C40exp[term][modes])
                    )
                    WinKernel[delta_k + delta_k_max, term, 5] += np.sum(
                        np.real(C42exp[term][modes])
                    )

        return WinKernel

    def load_resume_file(self, filename: str):
        """Load the window kernels from a file.
    ...
    def set_resume_file(self, filename: str):
        ...
        if self._resume_file is not None:
            try:
                ...
                self.logger.warning(
                    f"Loaded resume file {self._resume_file}. This might override your settings. See debug messages for more details on the loaded attributes."
                )
            except FileNotFoundError:
                self.logger.info(
                    f"File {self._resume_file} not found. Creating resume file."
                )
                utils.mkdir(os.path.dirname(self._resume_file))
                self.save(self._resume_file)

    @classmethod
    def from_window_kernels_file(cls: Any, filename: str):
