...
class PowerSpectrumMultipolesCovariance(base.MultipoleFourierCovariance):
    ...
    def alpha(self) -> float | None:
        ...
        if self._alpha is None:
            alpha = getattr(self.geometry, "alpha", None)
            return alpha
        return self._alpha
...
class GaussianCovariance(PowerSpectrumMultipolesCovariance):
    ...
    def __init__(self, geometry=None):
        PowerSpectrumMultipolesCovariance.__init__(self, geometry=geometry)
    ...
    def load_pypower(
        self, pypower, remove_shotnoise=None, set_shotnoise=False, naverage=1
    ):
    ...
    def _compute_cosmic_variance(self):
        ...
        if os.path.exists(filename):
            ...
            coefficients = base.SparseNDArray(
                shape_out=(3, 3, 3, 3, 3, 3, 3, 3), shape_in=(7, 7, 7, 7)
            )

            for l1, l2, l3, l4 in itt.product((0, 2, 4), repeat=4):
                for m1, m2, m3, m4 in itt.product(
                    *[np.arange(-l, l + 1, 2) for l in (l1, l2, l3, l4)]
                ):
                    for la in np.arange(np.abs(l1 - l4), l1 + l4 + 1, 2):
                        for lb in np.arange(np.abs(l2 - l3), l2 + l3 + 1, 2):
                            for ma, mb in itt.product(
                                *[np.arange(-l, l + 1, 2) for l in (la, lb)]
                            ):
                                value = np.float64(
                                    sympy.physics.wigner.gaunt(l1, l4, la, m1, m4, ma)
                    ...
                    for lc in np.arange(np.abs(l1 - l2), l1 + l2 + 1, 2):
                        for la in np.arange(np.abs(lc - l4), lc + l4 + 1, 2):
                            for ma, mc in itt.product(
                                *[np.arange(-l, l + 1, 2) for l in (la, lc)]
                            ):
                                value = np.float64(
                                    sympy.physics.wigner.gaunt(l1, l2, lc, m1, m2, mc)
        ...
        nmesh = 512
        windows_ab = base.SparseNDArray(
            shape_out=(7, 7), shape_in=(nmesh, nmesh, nmesh)
        )
        windows_cd = base.SparseNDArray(
            shape_out=(7, 7), shape_in=(nmesh, nmesh, nmesh)
        )
        ...
        for iell, ell in enumerate(np.arange(0, ellmax, 2)):
            for im, m in enumerate(np.arange(0, ell + 1, 2)):
                windows_ab[iell, im] = self.geometry["ab"].mesh(
                    ell=ell, m=m, shotnoise=False, fourier=True, threshold=1e-5
                )
                windows_cd[iell, im] = self.geometry["cd"].mesh(
                    ell=ell, m=m, shotnoise=False, fourier=True, threshold=1e-5
                )

        windows_prod = base.SparseNDArray(
            shape_out=(7, 7, 7, 7), shape_in=(nmesh, nmesh, nmesh)
        )

        for l1, l2 in itt.product((0, 2, 4), repeat=2):
            for m1, m2 in itt.product(*[np.arange(0, l + 1, 2) for l in (l1, l2)]):
                windows_prod[l1 // 2, l2 // 2, m1 // 2, m2 // 2] = (
                    windows_ab[l1 // 2, m1 // 2] * windows_cd[l2 // 2, m2 // 2]
                )
